...
Шаблон тренировки — заготовка с набором упражнений. Шаблоны бывают **системные**
(`created_by_user_id = null`, наполняются seed-скриптом, доступны всем только на чтение)
и **пользовательские** (`created_by_user_id` = текущий пользователь). Шаблон хранит
**только упорядоченный список упражнений** — без подходов/повторений/веса.

Все эндпоинты требуют `Authorization: Bearer <access_token>`.

### 7.1 `GET /api/workout-templates`

Список доступных шаблонов: системные + собственные шаблоны текущего пользователя.
...
[
  {
    "id": "0f3c1a2b-1111-2222-3333-444455556666",
    "created_by_user_id": null,
    "title": "День груди",
    "description": "Базовые упражнения на грудь.",
    "is_favorite": false,
    "exercises": [
      { "exercise_id": "a7f5d856-7b53-4e5f-bf6d-8f8ed7483b89", "order_index": 1 },
      { "exercise_id": "6f7dd56f-8188-4e93-aa62-5e2e453de0db", "order_index": 2 }
    ],
...
- `created_by_user_id` — `null` для системного шаблона, иначе UUID владельца
- `is_favorite` — лайкнут ли шаблон текущим пользователем
- `exercises` — отсортированы по `order_index` (с 1)
...
### 7.2 `GET /api/workout-templates/{template_id}`

Получение одного шаблона (своего или системного).

**Response 200** — формат как элемент в `GET /api/workout-templates`.

**Ошибки**

- `401` — нет/невалидный access-токен
- `404` — шаблон не найден или недоступен (чужой пользовательский)

---

### 7.3 `POST /api/workout-templates`
...
- `title` — строка, 1..255
- `description` — `string | null`, максимум 2000
- `exercises` — массив 0..100, каждый элемент `{ "exercise_id": "UUID" }`

**Правила**

- `title` уникален среди шаблонов пользователя (кейс-инсенситивно)
- `exercise_id` внутри шаблона не должны повторяться
- можно использовать системные и свои упражнения; нельзя — чужие пользовательские
- `order_index` не передаётся — backend ставит его по позиции в массиве

**Response 201** — формат как элемент в `GET /api/workout-templates` (`is_favorite=false`).
...
- `403` — попытка использовать чужое пользовательское упражнение
- `404` — одно или несколько упражнений не найдены
- `409` — шаблон с таким названием уже существует
- `422` — ошибка валидации

---

### 7.4 `PATCH /api/workout-templates/{template_id}`

Частичное обновление собственного шаблона. Передаются только изменяемые поля
(`title`, `description`, `exercises`). При передаче `exercises` список упражнений
...
- `401` — нет/невалидный access-токен
- `404` — шаблон не найден или не принадлежит пользователю (системные тоже `404`)
- `403` — попытка использовать чужое пользовательское упражнение
- `409` — шаблон с таким названием уже существует
- `422` — ошибка валидации (в т.ч. пустое тело)

---

### 7.5 `DELETE /api/workout-templates/{template_id}`

Удаление собственного шаблона.
...
- `401` — нет/невалидный access-токен
- `404` — шаблон не найден или не принадлежит пользователю (системные не удаляются)

---

### 7.6 `POST /api/workout-templates/{template_id}/favorite`

Добавить шаблон в личное избранное (лайк). Идемпотентно (повторный вызов не создаёт дубль).
Лайкать можно как свои, так и системные шаблоны.
...
- `401` — нет/невалидный access-токен
- `404` — шаблон не найден или недоступен

---

### 7.7 `DELETE /api/workout-templates/{template_id}/favorite`

Убрать шаблон из личного избранного. Идемпотентно (если не было в избранном — тоже `204`).
